...
export function LocationMarker({ x, y }: { x: string; y: string }) {
  return (
    ...
    >
      ...
      {/* View Cone */}
      <div
        className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-[100%] w-16 h-16 origin-bottom"
        style={{ transform: "translate(-50%, -100%) rotate(45deg)" }}
      >
        <div
          className="w-full h-full bg-gradient-to-t from-[var(--primary)]/30 to-transparent rounded-full blur-sm"
