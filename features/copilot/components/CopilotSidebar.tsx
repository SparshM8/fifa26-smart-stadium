...
export function CopilotSidebar({ className }: { className?: string }) {
  return (
    <div className={cn("flex flex-col h-full w-[300px] glass border-r border-white/5", className)}>
      ...
      <div className="p-4 border-b border-white/5 bg-[var(--accent-cyan)]/5">
        ...
        <div className="space-y-2">
          <div className="bg-black/40 rounded-lg p-2 border border-[var(--accent-cyan)]/20">
            <div className="text-xs text-white font-medium">Camera Feed Active</div>
            <div className="text-[10px] text-zinc-400 font-mono">Gate 4, Sector B (Node #401)</div>
          </div>
          <div className="bg-black/40 rounded-lg p-2 border border-white/10">
            <div className="text-xs text-white font-medium">IoT Sensor Array</div>
            <div className="text-[10px] text-zinc-400 font-mono">Turnstile through-put: 42/min</div>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4">
        <div className="text-xs font-bold font-mono text-zinc-500 uppercase tracking-wider mb-3">Today</div>
        <div className="space-y-1">
          {[
            ...
            <button key={i} className="w-full flex items-center gap-3 px-3 py-2 rounded-xl text-left text-sm text-zinc-300 hover:bg-white/5 hover:text-white transition-colors">
