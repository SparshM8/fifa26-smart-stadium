...
export function OpsKPIs() {
  const baseAttendance = typeof OPS_KPIS[0].value === "number" ? OPS_KPIS[0].value : 82450
  ...
  const tick = React.useCallback(() => {
    setAttendance(prev => Math.min(82500, prev + Math.floor(Math.random() * 5)))
  }, [])
    ...
    <section aria-label="Key performance indicators">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <MetricCard
          ...
          trend="up"
          trendValue="Spiking"
          icon={<Wifi className="w-5 h-5 text-[var(--accent-cyan)]" aria-hidden="true" />}
        />
        <MetricCard
          ...
          icon={<AlertTriangle className="w-5 h-5 text-[var(--danger)]" aria-hidden="true" />}
        />
        <MetricCard
          ...
          trend="neutral"
          trendValue="Deployed"
          icon={<UserCheck className="w-5 h-5 text-[var(--success)]" aria-hidden="true" />}
        />
      </div>
    </section>
  )
